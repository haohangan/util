...
def cal_acc_ct_async(start_date: dt.date, period: int):
    ...
    results = []
    df = gen_flow()
    load_dates = [start_date + dt.timedelta(days=num) for num in range(period)]
    with concurrent.futures.ProcessPoolExecutor(max_workers=7) as executor:
        future_to_task = {executor.submit(cal_ct, df, load_date): load_date for load_date in load_dates}
        print(type(future_to_task))
        for future in concurrent.futures.as_completed(future_to_task):
            print(type(future))
            task_id = future_to_task[future]
            try:
                # future.result() 获取任务的返回值
                # 如果任务执行过程中抛出异常，.result() 会重新抛出该异常
                result = future.result()
                results.append(result)
                print(f"收到结果: Task {task_id}")  # 打印部分结果
            except Exception as exc:
                print(f'Task {task_id} 生成了一个异常: {exc}')  # results.append(f"Task {task_id} failed")  # 记录失败
    # print(f"\n准备并发执行 {len(tasks_to_run)} 个任务...")
